...
async fn bootstrap_response_includes_active_skills_after_skill_upload() {
    let dir = unique_chain_dir();
    // Use a single REST router so that the skill upload and bootstrap share the
    // same in-memory skill registry — no cross-instance staleness.
    let router = rest_router(MentisDbServiceConfig::new(
        dir.clone(),
        "spawn-skill-test",
        StorageAdapterKind::Jsonl,
...
"#;
    ...
    // Bootstrap and verify `available_skills` lists the uploaded skill.
    let bootstrap = router
        .oneshot(
            Request::builder()
                .method("POST")
